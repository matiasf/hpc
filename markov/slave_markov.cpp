...
void slave(int grank) {
	...
	while(true) {      
	  ...
	  if (message.compare("BALANCE") == 0) {
	    ...
	    cerr << "Slave" << rank << ": Master message - " << masterMessage << " to " << cell.rank << endl;
    ...
    void addWord(string word) {
	    ...
	    //////////cerr << "Slave: Rank " << rank << endl;
	    //////////cerr << "Slave: Word 1 " << word1 << endl;
	    //////////cerr << "Slave: Word 2 " << word2 << endl;
	    for (vector<column>::iterator it1 = columns.begin(); it1 < columns.end(); it1++) {
		    if(word1.compare((*it1).word)==0) {
			    ...
			    if(notInWord2) {
			      //////////cerr << "Slave: Adding word 1 " << word2 << endl;
	    ...
	    if(notInWord1) {
	      //////////cerr << "Slave: Adding word 2 " << word2 << endl;
	      column* c = new column();
	      (*c).word = word1;
	      columns.push_back(*c);
	      rc = addWordToColumn(word2,rank);
...
routecell searchNextWord(string word) {
  ...
  for (vector<column>::iterator it1 = columns.begin(); it1 < columns.end(); it1++) {
    ...
    if(word.compare((*it1).word) == 0) {
      //cerr << "Slave" << rank << ": Finded word " << (*it1).word << endl;
